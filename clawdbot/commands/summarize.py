...
def load_enhanced_projects(output_path):
    ...
    try:
        ...
        for project in enhanced_projects:
            if project.get("clawdbot_analysis"):
...
def get_pdf_text_for_project(project):
    ...
    for mention in project.get('mentions', []):
        ...
        # Resolve path relative to clawdbot directory
        clawdbot_dir = Path(__file__).parent.parent
        text_file_path = clawdbot_dir / f"../outputs/files/{source}/{year}/{filename}.txt"
...
def enhance_project(project, prompt_template, config):
    """Add clawdbot_analysis to single project with PDF content."""
...
def main(argv=None):
    ...
    try:
        ...
        if newly_enhanced and newly_enhanced[0].get("clawdbot_analysis"):
