...
def get_adapter_function(adapter_name: str):
    """Dynamically import and return adapter function"""
    try:
        if adapter_name == "planning_commission_adapter":
            from ..adapters.planning_commission_fetch import fetch
            return fetch
        elif adapter_name == "infrastructure_adapter":
            from ..adapters.infrastructure_fetch import fetch
            return fetch
        elif adapter_name == "private_development_adapter":
            from ..adapters.private_development_fetch import fetch
            return fetch
        elif adapter_name == "public_projects_adapter":
            from ..adapters.public_projects_fetch import fetch
            return fetch
        else:
            logger.error(f"Unknown adapter: {adapter_name}")
            return None
    except ImportError as e:
        logger.error(f"Failed to import adapter {adapter_name}: {e}")
        return None
...
def run_observatory():
    ...
    for source in sources:
        ...
        try:
            ...
            error_msg = f"Adapter failed for {source_name}: {str(e)}"
            logger.error(error_msg)
            alert_pipeline_failure(source_name, error_msg)
