...
Usage:
    python moltbot.py <command> [options]
...
Examples:
    python moltbot.py run --source dia_board --year 2025
    python moltbot.py run --source dia_board --dry-run
    python moltbot.py status
    python moltbot.py clean --source dia_board
""")
...
def run_command(args):
    ...
    print(f"Molt Bot starting reference detection...")
    if args.dry_run:
    ...
    try:
        ...
        print(f"✓ Stored in outputs/annotations/molt/")
...
def status_command(args):
    ...
    annotations_dir = Path("outputs/annotations/molt")

    if not annotations_dir.exists():
        print("No annotations found. Run 'moltbot.py run' to start enrichment.")
        return 0

    print("Molt Bot Enrichment Status")
    print("=" * 30)
...
def clean_command(args):
    ...
    print("Molt Bot cleaning annotations...")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Molt Bot - Custodial Document Enrichment Agent")
